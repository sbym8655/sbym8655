...
class(cov_vax_admin$local_health_jurisdiction)
typeof(cov_vax_admin$local_health_jurisdiction)
...
summary(cov_vax_admin$local_health_jurisdiction)
range(cov_vax_admin$local_health_jurisdiction)
...
unique(cov_vax_admin$local_health_jurisdiction)
```
\newpage
## Milestone 3
...
df7 <- df6 %>%
  ...
  arrange(df6$local_health_jurisdiction) %>%
  drop_na(local_health_jurisdiction)%>%
  drop_na(age12_plus_population)
unique(df7$local_health_jurisdiction)
df8<-df7 %>%
  mutate(local_health_jurisdiction=recode(local_health_jurisdiction, "BERKELEY"="ALAMEDA",
                                          "PASADENA"="LOS ANGELES",
                                          "LONG BEACH"="LOS ANGELES"))
unique(df8$local_health_jurisdiction)
view(df8)
...
# Clean rows where county is NA. 
# Correct these 3 wrong names: PASADENA – Los Angeles, BERKELEY - Alameda, LONG BEACH – Los Angeles
# is.na() returns missing; !is.na() returns non-missing
# filter(df4$local_health_jurisdiction, !is.na())
# df5 <- filter(df4, local_health_jurisdiction !%in% c("NA")
# df5 <- filter(df4, local_health_jurisdiction == "KERN" | local_health_jurisdiction == "KINGS" |)

# Code above not working to clean "NA" rows, therefore brute force below to standardize counties across the two data sets.

df5 <- filter(df4, local_health_jurisdiction %in% c("KERN", "KINGS", "LAKE", "LASSEN", "LOS ANGELES",
                                                    ...
                                                    "ALAMEDA", "ALPINE", "SONOMA", "AMADOR",
                                                    "STANISLAUS", "SUTTER", "BUTTE", "CALAVERAS",
                                                    "TEHAMA", "COLUSA", "TRINITY", "TULARE",
                                                    "CONTRA COSTA", "DEL NORTE", "TUOLUMNE",
                                                    "VENTURA", "EL DORADO", "YOLO", "FRESNO",
                                                    "GLENN", "YUBA", "HUMBOLDT","IMPERIAL",
                                                    "INYO")) 
...
data(df_ca_co_demog)
variable.names <- c("No.", "name", "pop2012", "med_age")
var_desc <- c("County number", "County name", 
             "Population (2012)", "Median age")
var_type <- c(0, 1, 0, 0)
                          
linker <- build_linker(my.data = df_ca_co_demog, variable_description = var_desc, variable_type = var_type)
...
library(arsenal) 
table_one <- tableby(name ~ med_age + pop2012, data = df_ca_co_demog) 
summary(table_one, title = "Median Age by County and Population in 2012")
```
## Milestone 4
Visualizations
